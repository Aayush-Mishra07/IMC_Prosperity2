...
class Trader:
	...
	if state.position.get('AMETHYSTS') is not None:
		if -best_bid_amount + state.position[product] < -20 or best_bid_amount < -20:
			if state.position[product] > 0:
				best_bid_amount = -20 + state.position[product]
